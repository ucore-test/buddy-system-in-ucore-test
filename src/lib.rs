...
pub mod alloc{
    ...
    use super::{BuddyAllocator, LinkedListBuddy, UsizeBuddy};
...
pub extern "C" fn alloc()->NonNull<Page>{
    ...
    println!("hw test alloc");
    unsafe{
        let (point,size) = ALLOCATOR.allocate_type::<Page>().unwrap();
        println!("{}",size);
        println!("code in rust - point: {:?}",point);
        return point;
    }
